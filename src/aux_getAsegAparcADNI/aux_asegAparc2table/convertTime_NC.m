function [convert,time_conver_cens] = convertTime_NC(Tadni,group)
...
for i=1:length(subjects)
    ...
    if((length(labels_patient)==1 && diagn_patient(end)==group(1)) || diagn_patient(end)==group(1))%NC
        time_conver_cens(index)=time_patient(end);
    else
        for j=1:length(index)
            if(diagn_patient(j)~=group(1)) %MCI/dementia
                convert(index)=true;
